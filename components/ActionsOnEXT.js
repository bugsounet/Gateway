...
function ActionsOnEXT(that,noti,payload,sender) {
  switch(noti) {
    case "EXT_HELLO":
      helloEXT(that, payload)
      break
    case "EXT_PAGES-Gateway":
      if (sender.name == "EXT-Pages") Object.assign(that.GW["EXT-Pages"], payload)
      break
    case "EXT_GATEWAY":
      gatewayEXT(that, payload)
      break
    case "EXT_SCREEN-OFF":
    ...
    case "EXT_MUSIC-CONNECTED":
      ...
      connectEXT(that,"EXT-MusicPlayer")
      break
    case "EXT_MUSIC-DISCONNECTED":
      if (!that.GW["EXT-MusicPlayer"].hello) return console.log("[GATEWAY] Warn MusicPlayer don't say to me HELLO!")
      disconnectEXT(that,"EXT-MusicPlayer")
      break
    case "EXT_RADIO-CONNECTED":
      ...
      connectEXT(that,"EXT-RadioPlayer")
      break
    case "EXT_RADIO-DISCONNECTED":
      if (!that.GW["EXT-RadioPlayer"].hello) return console.log("[GATEWAY] Warn RadioPlayer don't say to me HELLO!")
      disconnectEXT(that,"EXT-RadioPlayer")
      break
    ...
    case "EXT_SPOTIFY-SCL_FORCED":
      ...
      if (that.GW["EXT-SpotifyCanvasLyrics"].forced && that.GW["EXT-Spotify"].remote && that.GW["EXT-Spotify"].play) connectEXT(that,"EXT-SpotifyCanvasLyrics")
      if (!that.GW["EXT-SpotifyCanvasLyrics"].forced && that.GW["EXT-SpotifyCanvasLyrics"].connected) disconnectEXT(that,"EXT-SpotifyCanvasLyrics")
      break
...
/** Scan GA Response **/
/**********************/
function gatewayEXT(that, response) {
